...
function getFCMServerKey(): string | null {
  // Use a proper FCM Server Key - the previous one was a VAPID key
  const fcmServerKey = Deno.env.get('FCM_SERVER_KEY') || "AAAA8vY8Qzs:APA91bGxample_server_key_here"

  if (!fcmServerKey || fcmServerKey.startsWith('BM31X')) {
    console.error('Variable d\'environnement FCM_SERVER_KEY manquante ou invalide (VAPID key détectée)')
    return null
  }
...
function getFirebasePublicConfig() {
  const apiKey = Deno.env.get('FIREBASE_API_KEY') || 'AIzaSyC1ew_x6gQvsTdnJ-gTqVot2XPCa2qKXX0'
  const authDomain = "airhost-d9c48.firebaseapp.com"
  const projectId = Deno.env.get('FCM_PROJECT_ID') || 'airhost-supabase'
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[FCM-PROXY DEBUG] Status de la réponse FCM:', response.status);
    console.log('[FCM-PROXY DEBUG] Headers de la réponse FCM:', Object.fromEntries(response.headers.entries()));
    ...
    // Vérifier si la réponse FCM indique une erreur
    if (!response.ok) {
      console.error('[FCM-PROXY ERROR] Erreur FCM API:', {
      ...
      throw new Error(`FCM API Error: ${response.status} - ${JSON.stringify(responseData)}`);
    }
