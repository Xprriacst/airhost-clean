...
async function handleGuestMessage(event, clientApiKey, hostId) {
  try {
    ...
    let { data: conversation, error: conversationError } = await supabase.from('conversations').select('id, unread_count, guest_name, lodgify_booking_id, lodgify_thread_uid, host_id').eq('lodgify_booking_id', parseInt(event.inbox_uid)).single();
...
if (simpleError || !simpleConversation || simpleConversation.length === 0) {
  ...
  const { data: newConversation, error: createError } = await supabase
    ...
    .insert({
      ...
      lodgify_booking_id: event.inbox_uid ? parseInt(event.inbox_uid) : null,
  ...
  const { data: lodgifyConfig, error: configError } = await supabase
    .from('lodgify_config')
    .select('api_key, host_id')
    .eq('host_id', hostId)
    .single();
  ...
  // Only update booking_id if it's missing and inbox_uid is a valid number
  if (!conversation.lodgify_booking_id && !isNaN(parseInt(event.inbox_uid))) {
    updateData.lodgify_booking_id = parseInt(event.inbox_uid);
    console.log('Also updating lodgify_booking_id to:', event.inbox_uid);
  }
