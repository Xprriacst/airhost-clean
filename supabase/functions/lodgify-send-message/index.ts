...
interface LodgifyBookingMessagePayload {
  subject: string
  message: string
  type: 'Owner' | 'Guest'
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { conversation_id, message, message_type = 'text' }: SendMessageRequest = await req.json()
    ...
    const lodgifyResponse = await fetch(`${lodgifyBaseUrl}/v1/reservation/booking/${bookingId}/messages`, {
      method: 'POST',
      headers: {
    ...
    let lodgifyResult: any = { message: 'Success - empty response' }
    ...
    if (responseText && responseText.trim()) {
      try {
        ...
        lodgifyResult = { message: 'Success', response: responseText }
      }
    } else {
  ...
  const booking =   await fetch(`${lodgifyBaseUrl}/v2/reservations/bookings/${bookingId}`, {
      method: 'GET',
      headers: {
        'X-ApiKey': lodgifyApiKey,
        'Content-Type': 'application/json'
    ...
    return new Response(JSON.stringify({
      ...
      lodgify_message_id: lodgifyResult.id
    }), {
      status: 200,
      headers: { 
        'Content-Type': 'application/json',
